function MID_SSVEP_static_disparity (SubjCode, runNumber)

% Inputs: 'SubjCode', anonymised unique subject code as a string, eg. 'RM'
%         runNumber, scalar giving the current run number.

% Displays only 2 interleaved conditions at random intervals (SOAs):
% 1. CD NEAR
% 2. CD FAR

% Both have a fixed disparity across their duration but dots are re-randomised with each video refresh. 
...
% Runs currently set to 2.5 minutes (RunDurationSec=150) ; this is half of what it was in the original
% experiment with the 4 different motion conditions (CD/IOVD towards and CD/IOVD away).

% towards: negative disparity
% away: positive disparity
% Modified from MID_SSVEP.m ; for use in a control EEG decoding experiment of static disparity only
% R Maloney, August 2019
...
% It shouldn't matter now what the frequency is because the sawtooth wave is now just a flat line:
% the max or min disparity for the full duration of the stimulus probe. 
parameters.CD_freq = 1; 
...
subjectData.experimentDescriptor = 'MID_SSVEP_static_disparity'; % NOTE change of output file name; RM 11/8/19
...
RunDurationSec = 150; % 2.5 min: was 300 sec previously (RM: 11/8/19)
...
% Let's assign each of the SOAs to one of the 2 conditions now. We want each to occur with equal probability, so
% let's cull cSOAs back until it's an exact multiple of 2:
% (note that previously this was 4: RM 11/8/19)
while mod(length(cSOAs), 2) > 0
    cSOAs = cSOAs(1:end-1);
end
%figure; bar(diff(cSOAs)); % plot distr of values before culling

% Now we can set up a vector that assigns each one of the cSOAs to one of the 2 conditions,
% in random order with equal probability
% The conditions are coded thus:
% 1 = CD NEAR
% 2 = CD FAR
AssignCond = repmat(1:2, 1, length(cSOAs)/2); % changed from 4 to 2: RM 11/8/19)
...
try % Start a try/catch statement, in case something goes awry with the PTB functions
    ...
    if useHardwareStereo
        ...
        DrawFormattedText(win,['Welcome ' SubjCode , ...
            '. \n \nPress '' 8 '' if the 3D stimulus is FAR from you,', ...
            '\n \nor press '' 2 '' if the 3D stimulus is NEAR to you.', ...
    ...
    while P <= length(AssignCond) % loop through all probes, including blanks
        ...
        switch AssignCond(P)
            ...
            case {1, 2} % It's any of the TWO stimulus categories (was 4: 11/8/19: RM)
        ...
        while ~probeEnd %This should keep iterating across stim frames until vbl >= probeEndVBL
            ...
            if keyIsDown %If they've pressed a button on keypad,
                ...
                % Correctness of the response will need to be inferred on the basis of the most recent probe stimulus.
                % We assume that the response will be made during a blank
                % period, so AssignCond(P) = 5. Use P-1 to determine the
                % most recent stimulus for providing audio feedback.
                % Only feedback for incorrect responses will be provided, as punishment! ;-D
                
                % NOTE: this will be left as-is for the static disparity task:
                % Subjects should simply respond 8 for AWAY and 2 for NEAR.
                % The fact that the Boolean statement for IOVD near/far remains (ie conditions 4 and 3)
                % shouldn't matter because they will never come up : RM 11/8/19
                
                if any(keyCode(TowardsMotion))            % Respond NEAR (ie TOWARDS)
                    Responses(overallPerEyeFrame,2) = -1; % -1 to indicate towards response.
                    
                    % If providing audio feedback:
                    if GiveFeedback
                        %if AssignCond(P-1) == 1 || AssignCond(P-1) == 3 % correct towards response
                            %Beeper(1000,0.4,0.1)
                        if AssignCond(P-1) == 2 || AssignCond(P-1) == 4 % incorrect towards response
                            Beeper(300,0.4,0.1)
                        end
                    end
                    
                elseif any(keyCode(AwayMotion))           % Respond FAR (ie AWAY)
                    Responses(overallPerEyeFrame,2) = 1;  % +1 to indicate away response.
                    
                    % If providing audio feedback:
                    if GiveFeedback
                        %if AssignCond(P-1) == 2 || AssignCond(P-1) == 4 % correct away response
                        %Beeper(1000,0.4,0.1)
                        if AssignCond(P-1) == 1 || AssignCond(P-1) == 3 % incorrect away response
                            Beeper(300,0.4,0.1)
                        end
                    end
...
MotionSwitch = [0 1; 0 0]; % 1 1; 1 0]; : Only include IOVD: RM 11/8/19
...
if MotionSwitch(Probe, 1) == 0
    ...
    % *** 11/8/19 (RM) *** : Re-assign the disparity to the max or min value 
    % This ensures the stimulus probe runs for the same duration as the previous experiment
    % BUT the DISPARITY remains fixed for the whole duration.
    % Though the dot positions are still updated on every frame.
    CDparams.SineWave =  DirnConstant * CDparams.disparity * ones(1, length(CDparams.t));
    ...
    % *** NOTE: *** in the static disparity decoding version of this experiment; the IOVD condition should never be
    % called and hence neither will the function below : RM 11/8/19
